...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val firstCol = StructField(columnNames.head, StringType, nullable = false)
  ...
  /** @return An RDD Row compatible with the schema produced by `dfSchema`
    ...
    */
  def row(line: List[String]): Row = Row.fromSeq(line.head.toString :: line.tail.map(x => x.toDouble))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    var primaryNeeds = columnNames.filter(p => p.startsWith("t01") ||
                                               p.startsWith("t03") ||
                                               p.startsWith("t11") ||
                                               p.startsWith("t1801") ||
                                               p.startsWith("t1803"))
    var workingActivities = (columnNames diff primaryNeeds).filter(p => p.startsWith("t05") ||
                                                                   p.startsWith("t1805"))
    var others = (columnNames diff primaryNeeds diff workingActivities).filter(p =>
                      p.startsWith("t02") ||
                      p.startsWith("t04") ||
                      p.startsWith("t06") ||
                      p.startsWith("t07") ||
    ...
    (primaryNeeds.map(new Column(_)), workingActivities.map(new Column(_)), others.map(new Column(_)))
  }
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
      summed
        ...
        .agg(round(avg($"primaryNeeds"), 1), round(avg($"work"), 1), round(avg($"other"), 1)).orderBy("working", "sex", "age")
  }
  ...
  /** @return SQL query equivalent to the transformation implemented in `timeUsageGrouped`
    ...
    */
  def timeUsageGroupedSqlQuery(viewName: String): String = "SELECT working, sex, age, round(avg(primaryNeeds),1), round(avg(work), 1), round(avg(other), 1) FROM "+viewName+" GROUP BY working, sex, age ORDER BY working, sex, age"
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed
      ...
      .map(p => new TimeUsageRow(p._1._1, p._1._2, p._1._3, Math.round(p._2*10.0)/10.0, Math.round(p._3*10.0)/10.0, Math.round(p._4*10.0)/10.0))
